...
public class MainActivity extends AppCompatActivity {
    //public static final int BOOK_DETAILS_ACTIVITY_REQUEST_CODE = 1;
    ...
    private void fetchBooksData(String query) {
        ...
        booksApiCall.enqueue(new Callback<BookContainer>() {
            @Override
            public void onResponse(Call<BookContainer> call,@NonNull Response<BookContainer> response) {
                setupBookListView(response.body().getBookList());
            }

            @Override
            public void onFailure(Call<BookContainer> call,@NonNull Throwable t) {
    ...
    private class BookHolder extends RecyclerView.ViewHolder {
        ...
        public void bind(Book book) {
            if(book!=null&&checkNullOrEmpty(book.getTitle())&&book.getAuthors()!=null){
                ...
                View itemContainer = itemView.findViewById(R.id.list_item_book);
                if(book.getCover()!=null){
                    Picasso.with(itemView.getContext()).load(IMAGE_URL_BASE + book.getCover()+"-S.jpg").placeholder(R.drawable.ic_book).into(bookCover);
                }
                else {
